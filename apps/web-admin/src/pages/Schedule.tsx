...
): { name: string; code: string | null; color: string | null; ph: boolean } {
  if (teamId && teamsById.has(teamId)) {
    const tm = teamsById.get(teamId)!;
    return { name: tm.name, code: tm.short_code, color: tm.color, ph: false };
  }
  return { name: placeholder ?? '—', code: null, color: null, ph: true };
}
...
function MatchRow({
  ...
  return (
    <div className={`mrow ${isFinal ? 'mrow--final' : ''}`}>
      ...
      <Crest code={home.code} color={home.color} size={28} />
      ...
      <Crest code={away.code} color={away.color} size={28} />
